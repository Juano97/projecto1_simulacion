from generador_VA_exp import gen_VA_exp as gVAe
from generador_VA_normal import gen_VA_normal as gVAn
import numpy as np
import sys

class Simulacion:    
    def __init__(self, T = 57600):
        #Variable de tiempo (tomaremos todos los tiempos del problema en minutos)
        self.T = T #24*60 1 dia
        self.t = 0
    ...
    def simulacion_evento(self):
        self.t = self.t_arribo = gVAe(8*60).__round__()
    ...
    def suc_arribo(self): #_lambda = 8*60
        ...
        self.cant_arribos+=1
        self.t_arribo = self.t + gVAe(8*60).__round__()
        if(self.muelles.__contains__(True) and self.remolcador == 0):
            self.remolcador = 2
            self.t_remolcador = self.t + gVAe(2*60).__round__()
        else:
            self.cola_puerto+=1
            ...
            if self.muelles[i] and muelle:
                ...
                if remolcador:
                    if self.cola_puerto and self.muelles.__contains__(True):
                        self.t_remolcador = self.t + gVAe(15).__round__()
                        self.remolcador = 4
                    else:
                ...
                self.muelles[i] = True
                self.t_remolcador = self.t + gVAe(60).__round__()
                self.remolcador = 3
